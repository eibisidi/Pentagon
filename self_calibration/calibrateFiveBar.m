...
G = acos(COSG);
Jk_formula = [diff(G, L11), diff(G, L12), diff(G, L21),  diff(G, L22), diff(G, D), diff(G, DELTA1), diff(G, DELTA2)];
...
vactual_less = [vactual(1); vactual(2); vactual(3); vactual(4); vactual(5); vactual(8); vactual(9);];
m = 7;
offset_error = 0.001;
...
vreal = [vnom(1); vnom(2); vnom(3); vnom(4); vnom(5); vnom(8); vnom(9)];
for i = 1:6
    for k = 1 : n %populate matrix J ; estimate est vector
